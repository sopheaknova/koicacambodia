...
class o2_video_widgets extends WP_Widget {
	function o2_video_widgets() {
		...
		$widget_ops = array('classname' => 'koica-videos','description' => __('Widget display viddeo support Youtube, vimeo, dailymotion','theme'));
/*		$control_ops = array( 'twitter name' => 'momizat', 'count' => 3, 'avatar_size' => '32' );
*/		
		$this->WP_Widget('koica-videos',__('KOICA - Videos','theme'),$widget_ops);
	...
	function widget( $args, $instance ) {
		...
		$type = $instance['type'];
		$id = $instance['id'];
		$more_videos_url = $instance['more_videos_url'];
...
?>
	...
	<?php } ?>
    <a href="<?php echo $more_videos_url; ?>" class="archives">Watch more videos</a>
<?php 
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['more_videos_url'] = $new_instance['more_videos_url'];
...
<option <?php if ( 'Dialymotion' == $instance['type'] ) echo 'selected="selected"'; ?>>Dialymotion</option>
	...
	<label for="<?php echo $this->get_field_id( 'id' ); ?>"><?php _e('Video ID:', 'theme') ?></label>
        ...
        <p>
	<label for="<?php echo $this->get_field_id( 'more_videos_url' ); ?>"><?php _e('Watch more videos url:', 'theme') ?></label>
		<input id="<?php echo $this->get_field_id( 'more_videos_url' ); ?>" name="<?php echo $this->get_field_name( 'more_videos_url' ); ?>" value="<?php echo $instance['more_videos_url']; ?>" class="widefat" />
		</p>
